...
locallevel_pop_props <- function(){
    ...
    for (la in laNames){
        ...
        # p.agesex.agegrp <- aggregate(p.agesex ~ rsex + agegrp,
        #                              data=p.res, FUN=sum)
        #
        # agesexethgrp.agegrp <- merge(p.agesex.agegrp, ethnicityLA[ ,c("rsex","age","ethnic2","p.ethngrp")],
        #                              by.x=c("agegrp","rsex"), by.y=c("age", "rsex"), all = TRUE)
        #
        # stopifnot(sum(agesexethgrp.agegrp[agesexethgrp.agegrp$rsex=="Men" & agesexethgrp.agegrp$agegrp=="16 to 17", "p.ethngrp"])==1)
